...
class PathfindingHelper extends Object3D {
  ...
  setPath(path) {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      //   path[i].z
      // );
      pointArr.push(path[i].x, path[i].y, path[i].z);
    }
    lineGeo.setPositions(pointArr);
    // 4. 设置材质分辨率
    this._pathLineMaterial.resolution.set(
      window.innerWidth,
      window.innerHeight
    );
    let line = new Line2(lineGeo, this._pathLineMaterial);
    line.computeLineDistances();
    this._pathMarker.add(line);
